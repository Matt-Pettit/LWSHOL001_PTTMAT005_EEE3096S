...
int main(void)
{
  ...
  while (1)
  {
	...
	char adc_val_string[24];
	sprintf(adc_val_string, "ADC: %d", adc_val);
	writeLCD(adc_val_string);

//getting the CCR VALUE
...
//handingling debounce
	if ((HAL_GetTick() - lastPressTime) >= 5)
	{
